...
from models.policy_networks import Ensemble_Policy_Network, TanhGaussianPolicy
from components.distributions import Delta
import components.pytorch_util as ptu

class Kfold_SAC(BaseTrainer):

    def init_model(self):
        self.print("init Kfold SAC model")
        ...
        self.policy = Ensemble_Policy_Network(
            args=self.args,
            obs_dim=self.expl_env.observation_space.shape[0],
            action_dim=self.expl_env.action_space.shape[0],
        ).to(self.args.device)
        self.expl_policy = TanhGaussianPolicy(
            self.args,
        ...
        self.expl_policy_optimizer = torch.optim.Adam(
    ...
    def eval_action(self, obs: torch.Tensor) -> torch.Tensor:
        ...
        with torch.no_grad():
            deterministic_dist = Delta(self.expl_policy(obs).mle_estimate())
    ...
    def compute_target(self, s_, alpha):
        b, k, s_dim = s_.shape[0], s_.shape[1], s_.shape[2]
        s_new = s_.reshape(b*k, s_dim)  # (bk, s)
        a_dim = self.expl_env.action_space.shape[0]
        with torch.no_grad():
            next_dist = self.policy(s_new)
            _, next_log_pi = next_dist.rsample_and_logprob()  # (bk, k)
            next_log_pi = next_log_pi.view(b, k, k).unsqueeze(dim=-1)  # (b, k, k, 1)
            next_dist_correspond = self.policy(s_)
            next_action, _ = next_dist_correspond.rsample_and_logprob() # (b, k, a)
            next_action = next_action.reshape(b*k, a_dim)
            target_pred = self.q_target(s_new, next_action)   # (bk, k, 1)
            target_pred = target_pred.view(b, k, k, 1)
            next_state_value = target_pred - alpha * next_log_pi  # (b, k, k, 1)
            next_state_value = next_state_value.mean(dim=2)       # (b, k, 1)
    ...
    def learn_on_batch(self, *args):
        ...
        expl_dist = self.expl_policy(s_new)
        ...
        expl_policy_loss = (alpha*log_pi - q_mean).mean()

        kfold_q_mean = (self.q_learner(s, a).sum(dim=1, keepdim=True) - self.q_learner(s, a)) / (k-1)  # (b, k, 1)
        dist = self.policy(s)
        new_action, new_log_pi = dist.rsample_and_logprob()  # (b, k, a) (b, k)
        new_log_pi = new_log_pi.unsqueeze(dim=-1)   # (b, k, 1)
        policy_loss = (alpha*new_log_pi - kfold_q_mean).mean()

        # Q loss
        next_state_value = self.compute_target(s_, alpha)  # (b, k, 1)
        ...
        expl_policy_loss.backward(retain_graph=True)
        ...
        # log stats
        with torch.no_grad():
            self.exp_path['batch_stats[t,q_loss,policy_loss,expl_policy_loss]'].csv_writerow(
                [self.t, qf_loss.mean().item(), policy_loss.item(), expl_policy_loss.item()]
            )
